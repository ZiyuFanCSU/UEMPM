...
from utils.evaluate_predict import metric as utils_evaluate_metric
from utils.evaluate_predict import metric_multitask as utils_evaluate_metric_multitask
from utils.evaluate_predict import metric_reg as utils_evaluate_metric_reg
from utils.evaluate_predict import metric_reg_multitask as utils_evaluate_metric_reg_multitask
...
def train_one_epoch_multitask(model, optimizer, data_loader, criterion, weights, device, epoch, task_type):
    ...
    sample_num = 0
    data_loader = tqdm(data_loader, disable=True)
    for step, data in enumerate(data_loader):
        smiles,s_m, labels,s = data
        smiles,s_m, labels = smiles.to(device),s_m.to(device), labels.to(device)
        sample_num += smiles.shape[0]

        pred = model.forward(smiles,s_m)
...
def evaluate_on_multitask(model, data_loader, criterion, device, epoch, task_type="classification", return_data_dict=False):
    ...
    for step, data in enumerate(data_loader):
        ...
        if len(pred) == 1:
            y_true.append(labels.view(pred.shape).unsqueeze(0))
            y_scores.append(pred.unsqueeze(0))
        else:
