    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(
        ...
        if no_encoder_attn:
            ...
            self.attn_attn = self.build_encoder_attention(self.embed_dim, args)
            self.attn_attn2 = self.build_encoder_attention(self.embed_dim, args)
        ...
        self.need_attn = True
        # 用于融合后的线性层，输出影响力度
        self.output_layer = nn.Linear(self.embed_dim, 2)
    ...
    def forward(
        ...
        if self.encoder_attn is not None and encoder_out is not None:
            ...
            # 4. 计算条件1和条件2对 x 的影响力度
            xx1, _ = self.attn_attn(
                query=x,
                key=encoder_out,
                value=encoder_out,
                key_padding_mask=encoder_padding_mask,
            ...
            combined_output = xx1 + xx2 + residual
            influence = self.output_layer(combined_output)  # [batch, len, 2]
            # 6. 使用 softmax 保证最后一维的和为 1
            influence = F.softmax(influence, dim=-1)  # [batch, len, 2]

            influence_x = influence[:, :, 0].unsqueeze(-1)  # [batch, len, 1]，表示 x 的权重
            influence_x2 = influence[:, :, 1].unsqueeze(-1)  # [batch, len, 1]，表示 x2 的权重
            
            # 分别对 x 和 x2 进行加权
            weighted_x = influence_x * x1 # [batch, len, hidden_dim]
            weighted_x2 = influence_x2 * x2  # [batch, len, hidden_dim]
            
            # 最终表征是 x 和 x2 的加权和
            x = weighted_x + weighted_x2  # [batch, len, hidden_dim]
